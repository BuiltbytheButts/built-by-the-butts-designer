...
function buildGuideVisuals(crosscuts, border, lamination) {
  ...
  const assignedCount = Math.max(0, border.selectedRows);
  const assignedSize = Math.min(52, 340 / Math.max(assignedCount, 1));
  const assignedStart = 210 - assignedCount * assignedSize / 2;
  const assignedLaminates = Array.from({ length: assignedCount }, (_, index) => {
  ...
  const edgeRipShapes = edgeRipSelected ? `<polygon points="145,95 178,62 178,128" fill="${guideWood(state.edgeWood)}"/><polygon points="275,95 242,62 242,128" fill="${guideWood(state.edgeWood)}"/>` : '';
  const edgeRipWords = edgeRipSelected ? ` Then remove only the two specified corner areas to a ${number(state.edgeInset).toFixed(3)} in depth and replace them with ${WOODS[state.edgeWood]?.name || state.edgeWood}.` : '';
  ...
  const steps = [
    ...
    { title: 'Glue and flatten the laminated blank', text: `Apply glue evenly, clamp across the full blank, then flatten it. Verify the blank is square and at least ${lamination.recommended.toFixed(3)} in in the required direction before continuing.`, svg: guideSvg(`${squareStripBlocks}${guideArrow(112,95,148,95,'clamp')}${guideArrow(308,95,272,95,'clamp')}<rect x="145" y="30" width="130" height="130" fill="none" stroke="#222" stroke-width="2"/>`, 'Clamped square laminated blank') },
    { title: edgeRipSelected ? 'Make the 45° cuts and Edge Rip' : 'Make the 45° cuts', text: `On the square laminate, mark a dotted line from the center of one edge to the center of the next edge. This center-to-center line establishes the 45° cut.${edgeRipWords}`, svg: guideSvg(`${squareStripBlocks}<rect x="145" y="30" width="130" height="130" fill="none" stroke="#222" stroke-width="2"/>${edgeRipShapes}<line x1="210" y1="30" x2="275" y2="95" class="guide-center-cut"/><circle cx="210" cy="30" r="4" class="guide-center-point"/><circle cx="275" cy="95" r="4" class="guide-center-point"/><text x="210" y="181" text-anchor="middle">Center to center = 45°${edgeRipSelected ? ' · Edge Rip selected' : ''}</text>`, edgeRipSelected ? 'Square laminate with center-to-center 45-degree cut and selected Edge Rip' : 'Square laminate with center-to-center 45-degree cut') },
    ...
    { title: 'Lay out every assigned laminate', text: `Keep the pieces in order. The build uses ${assignedCount} complete laminated row${assignedCount === 1 ? '' : 's'}, shown together in one line below. Rotate every second piece 180° and mirror adjacent faces so the bands close into diamonds.`, svg: guideSvg(`<text x="210" y="32" text-anchor="middle">${assignedCount} laminate${assignedCount === 1 ? '' : 's'} assigned to this build</text>${assignedLaminates}${guideArrow(115,155,185,155,'alternate rotation')}<path d="M300 158 A20 20 0 1 1 326 139" fill="none" class="guide-arrow" marker-end="url(#guide-arrowhead)"/>`, 'All assigned square laminates shown in one line') },
