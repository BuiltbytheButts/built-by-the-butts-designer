...
function stripScheduleEngineering() {
  return DiamondManufacturing.stripScheduleLaminationPlan({
    stripWidths: activeStrips().map(strip => strip.width),
    requiredLaminationSize: laminationRequirement().recommended
  });
}
...
function renderEngineering() {
  ...
  $('crosscutCountHelp').textContent = `${x.crosscutCount} × ${x.roughCrosscut.toFixed(3)} in rough crosscuts + ${Math.max(0, x.crosscutCount - 1)} × ${x.bladeKerf.toFixed(3)} in kerf = ${x.requiredBlankLength.toFixed(3)} in rough blank; yields approx. ${x.achievableLength.toFixed(3)} in finished.`;
...
function renderMetrics() {
  ...
  $('stripTotalMetric').textContent = `${stripPlan.total.toFixed(4)} in`;
  $('stripTotalHelp').textContent = stripPlan.matches
    ? `Required ${stripPlan.required.toFixed(4)} in — matched ✓`
    : `Required ${stripPlan.required.toFixed(4)} in`;
  const stripDifference = Math.abs(stripPlan.difference);
  const stripWarning = $('stripTotalWarning');
  stripWarning.hidden = stripPlan.matches;
  stripWarning.textContent = stripPlan.matches
    ? ''
    : `⚠ Strip schedule is ${stripDifference.toFixed(4)} in ${stripPlan.difference < 0 ? 'short' : 'over'} for the required pre-45° lamination size. ${stripPlan.difference < 0 ? 'Add' : 'Remove'} ${stripDifference.toFixed(4)} in across the strip schedule.`;
  $('stripTotalCard').classList.toggle('engineering-has-warning', !stripPlan.matches);
  const laminationWarning = $('laminationWarning');
  laminationWarning.hidden = stripPlan.matches;
  laminationWarning.textContent = stripPlan.matches
    ? ''
    : `⚠ Strips total ${stripPlan.total.toFixed(4)} in — ${stripDifference.toFixed(4)} in ${stripPlan.difference < 0 ? 'short' : 'over'} of the ${stripPlan.required.toFixed(4)} in pre-45° requirement.`;
  $('laminationMetricCard').classList.toggle('metric-has-warning', !stripPlan.matches);
...
function buildGuideVisuals(crosscuts, border, lamination) {
  ...
  const squareStripBlocks = strips.map(strip => {
    const width = 130 * number(strip.width) / total;
    const block = `<rect x="${squareStripX.toFixed(2)}" y="30" width="${width.toFixed(2)}" height="130" fill="${guideWood(strip.wood)}"/>`;
    squareStripX += width;
    return block;
  }).join('');
  ...
  const requiredLaminatedRows = Math.max(1, border.selectedRows);
  const displayedLaminatedRows = Math.min(4, requiredLaminatedRows);
  const dryFitHalfSize = 44;
  const dryFitRowWidth = displayedLaminatedRows * dryFitHalfSize * 2;
  const dryFitRowStart = 210 - dryFitRowWidth / 2;
  const dryFitRowEnd = 210 + dryFitRowWidth / 2;
  const dryFitCenters = Array.from({ length: displayedLaminatedRows }, (_, index) => dryFitRowStart + dryFitHalfSize + index * dryFitHalfSize * 2);
  const dryFitRadius = dryFitHalfSize * Math.SQRT2;
  const dryFitCenterY = 84;
  const dryFitTop = dryFitCenterY - dryFitHalfSize;
  const dryFitBottom = dryFitCenterY + dryFitHalfSize;
  const dryFitCoreHalf = dryFitRadius * 0.412;
  const dryFitCoreHalfHeight = dryFitRadius * 0.602;
  ...
  const dryFitPieceBodies = dryFitCenters.map((center, index) => {
    ...
    const pieceStrips = strips.map(strip => {
      ...
      const shape = `<rect data-guide-part="dry-fit-strip" x="${pieceStripX.toFixed(2)}" y="${(dryFitCenterY - dryFitRadius).toFixed(2)}" width="${width.toFixed(2)}" height="${(dryFitRadius * 2).toFixed(2)}" fill="${guideWood(strip.wood)}"/>`;
      pieceStripX += width;
      return shape;
    }).join('');
  ...
  const dryFitLabels = dryFitCenters.map((center, index) => `<text x="${center}" y="148" text-anchor="middle">Row ${index + 1}</text>`).join('');
  ...
  const rowDisplayLabel = requiredLaminatedRows > displayedLaminatedRows
    ? `Rows 1–${displayedLaminatedRows} of ${requiredLaminatedRows} shown`
    : `All ${requiredLaminatedRows} required row${requiredLaminatedRows === 1 ? '' : 's'} shown`;
  const dryFitRow = `<defs>${picture8DryFit.definitions}</defs><g data-guide-part="dry-fit-row" data-guide-required-rows="${requiredLaminatedRows}" data-guide-shown-rows="${displayedLaminatedRows}">${dryFitPieces}${dryFitJoints}</g><text x="210" y="178" text-anchor="middle">${rowDisplayLabel} · continue the alternating perspective through Row ${requiredLaminatedRows}</text>`;
  ...
  const masterProfileBorders = state.includeBorders ? border.bands.map(band => {
    ...
    return `<rect data-guide-part="master-blank-border" x="${dryFitRowStart.toFixed(2)}" y="${topY.toFixed(2)}" width="${dryFitRowWidth.toFixed(2)}" height="${bandHeight.toFixed(2)}" fill="${guideWood(band.wood)}"/><rect data-guide-part="master-blank-border" x="${dryFitRowStart.toFixed(2)}" y="${bottomY.toFixed(2)}" width="${dryFitRowWidth.toFixed(2)}" height="${bandHeight.toFixed(2)}" fill="${guideWood(band.wood)}"/>`;
  }).join('') : '';
  ...
  const assembledMasterProfile = `<defs>${picture9DryFit.definitions}</defs><text x="210" y="16" text-anchor="middle">Required Length: ${crosscuts.requiredBlankLength.toFixed(3)} in</text><g data-guide-part="master-blank-profile" data-guide-required-rows="${requiredLaminatedRows}" data-guide-shown-rows="${displayedLaminatedRows}">${picture9DryFit.bodies}${masterProfileBorders}${masterProfileSeams}<rect x="${dryFitRowStart.toFixed(2)}" y="${(dryFitTop - masterProfileBorderOffset).toFixed(2)}" width="${dryFitRowWidth.toFixed(2)}" height="${(88 + masterProfileBorderOffset * 2).toFixed(2)}" fill="none" stroke="#33261e" stroke-width="2"/></g><text x="210" y="154" text-anchor="middle">${rowDisplayLabel} in the joined end profile</text><text x="210" y="175" text-anchor="middle">Glue all ${requiredLaminatedRows} required rows through the full required length</text>`;
  ...
  const masterCrosscutDiagram = `${masterTopSurface}${topCrosscutLines}<path d="M389 ${masterTopY} V${(masterTopY + cutGap).toFixed(2)}" class="guide-dimension"/><text x="394" y="${(masterTopY + cutGap / 2 + 4).toFixed(2)}">${crosscuts.roughCrosscut.toFixed(3)} in</text><text x="210" y="174" text-anchor="middle">Top surface of the same ${requiredLaminatedRows}-row master blank</text>`;
  ...
  const steps = [
    ...
    ...edgeRipSteps,
    { title: 'Dry-fit the 45° cut pieces', text: `This build requires ${requiredLaminatedRows} laminated row${requiredLaminatedRows === 1 ? '' : 's'}. The diagram shows ${requiredLaminatedRows > displayedLaminatedRows ? `the first ${displayedLaminatedRows}` : `all ${requiredLaminatedRows}`}; continue the same alternating perspective through Row ${requiredLaminatedRows}. Flip odd-numbered rows end-for-end so matching diagonal bands form half-diamonds${edgeRipSelected ? ` and the ${edgeRipSpecies} Diamond Accent faces align at each seam` : ''} before gluing.`, svg: guideSvg(dryFitRow, `Representative flat-face dry fit showing ${displayedLaminatedRows} of ${requiredLaminatedRows} required laminated rows with matching half-diamonds and aligned ${edgeRipSpecies} Diamond Accent faces`) },
    { title: state.includeBorders ? 'Glue the borders before crosscutting' : 'Prepare the full master blank', text: state.includeBorders ? `Glue all ${requiredLaminatedRows} aligned laminated rows edge-to-edge, then attach the complete border schedule to both long outside edges. The diagram shows ${displayedLaminatedRows} representative row${displayedLaminatedRows === 1 ? '' : 's'} for readability. Verify at least ${crosscuts.requiredBlankLength.toFixed(3)} in remains usable before crosscutting.` : `Glue all ${requiredLaminatedRows} aligned laminated rows edge-to-edge into the full master blank. The diagram shows ${displayedLaminatedRows} representative row${displayedLaminatedRows === 1 ? '' : 's'} for readability. Verify the repeated Picture 8 orientation and at least ${crosscuts.requiredBlankLength.toFixed(3)} in of usable length before crosscutting.`, svg: guideSvg(assembledMasterProfile, `Representative joined master-blank end profile showing ${displayedLaminatedRows} of ${requiredLaminatedRows} required rows${state.includeBorders ? ' with the selected borders attached' : ''}`) },
    { title: 'Mark the crosscuts from the top view', text: `View the same ${requiredLaminatedRows}-row master blank from above and run each dotted cut line fully across its width. Mark ${crosscuts.crosscutCount} crosscuts at approximately ${crosscuts.roughCrosscut.toFixed(3)} in spacing, allowing for the editable ${crosscuts.bladeKerf.toFixed(3)} in blade kerf.`, svg: guideSvg(masterCrosscutDiagram, `Top surface of the aligned ${requiredLaminatedRows}-row master blank with every calculated crosscut marked`) },
