...
function buildGuideVisuals(crosscuts, border, lamination) {
  ...
  const edgeRipStep = edgeRipSelected ? [{
    title: 'Complete the Edge Rip after the 45° cuts',
    text: `Keep the completed 45° pieces in order. Only now, cut across the ${edgeRipSpecies} area to the selected ${number(state.edgeInset).toFixed(3)} in depth and install the ${edgeRipSpecies} replacement stock.`,
    svg: guideSvg(`<polygon points="55,42 315,42 360,78 100,78" fill="${guideWood(strips[0]?.wood)}" stroke="#33261e" stroke-width="2"/><polygon points="55,42 125,42 170,78 100,78" fill="${guideWood(state.edgeWood)}"/><line x1="125" y1="35" x2="170" y2="86" class="guide-center-cut"/><text x="147" y="28" text-anchor="middle">CUT ACROSS ${edgeRipSpecies.toUpperCase()}</text><polygon points="55,112 315,112 360,148 100,148" fill="${guideWood(strips[0]?.wood)}" stroke="#33261e" stroke-width="2"/><polygon points="245,112 315,112 360,148 290,148" fill="${guideWood(state.edgeWood)}"/><line x1="245" y1="105" x2="290" y2="156" class="guide-center-cut"/><text x="267" y="99" text-anchor="middle">CUT ACROSS ${edgeRipSpecies.toUpperCase()}</text>`, `Edge Rip cut across the ${edgeRipSpecies} portion after the 45-degree cuts`)
  }] : [];
...
function renderWorkshopPlan() {
  ...
  const edgeRipChecklist = number(state.edgeInset) > 0.0005
    ? `<li>After all four 45° cuts are complete, perform the ${number(state.edgeInset).toFixed(3)} in Edge Rip across the ${WOODS[state.edgeWood]?.name || state.edgeWood} section and install the replacement stock.</li>`
    : '';
