...
function buildGuideVisuals(crosscuts, border, lamination) {
  ...
  const steps = [
    ...
    { title: 'Mark the crosscuts from the top view', text: `Prepare at least ${crosscuts.requiredBlankLength.toFixed(3)} in of master blank. From the top, mark ${crosscuts.crosscutCount} crosscuts at approximately ${crosscuts.roughCrosscut.toFixed(3)} in between cut lines, allowing for the editable ${crosscuts.bladeKerf.toFixed(3)} in blade kerf.`, svg: guideSvg(`<rect x="35" y="65" width="350" height="62" fill="${guideWood(strips[0]?.wood)}"/>${pieces}<rect x="45" y="51" width="${Math.max(4, pieceWidth * .12)}" height="90" fill="url(#guide-kerf)"/><path d="M55 146 H${(55 + pieceWidth - 3).toFixed(2)}" class="guide-dimension"/><text x="${(55 + (pieceWidth - 3) / 2).toFixed(2)}" y="163" text-anchor="middle">${crosscuts.roughCrosscut.toFixed(3)} in</text><text x="210" y="28" text-anchor="middle">Top view · ${crosscuts.crosscutCount} crosscut lines</text>`, 'Top view of the master blank with crosscut lines and spacing') },
